...
bool CprHTTP::send_imen(std::string url, std::vector<unsigned char> *buffer, int64_t timestamp) {
    ...
    auto response = cpr::Post(
        cpr::Url{url},
        cpr::Multipart{{"image", cpr::Buffer{buffer->begin(), buffer->end(), filename}}});
