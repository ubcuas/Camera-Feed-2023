...
std::unique_ptr<ImageData> AravisCamera::get_image() {
  ...
  if (arvBuffer != NULL) {
    ...
    int64_t timeStamp = arv_buffer_get_timestamp(
        arvBuffer);  // not sure if this one is better than the next line
...
//     const char* string is the name of the node based on GenIcam standard
//     naming
